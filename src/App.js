...
import {Box, Flex, Button, useDisclosure, Table, Thead, Tr, Tbody, Td, useBreakpointValue, Th} from "@chakra-ui/react"
              ...
              <Tbody>
                {data.map(({name, email}, index) =>(
                  <Tr key={index} cursor="pointer" _hover={{ bg: "grey.100"}}>
                    ...
                    <Td p={0}>
                      <EditIcon fontSize={20} onClick={() => [setDataEdit({name, email, index}), onOpen(),]}></EditIcon>
                    </Td>
                    <Td p={0}>
                      <DeleteIcon fontSize={20} onClick={() => handleRemove(email)}></DeleteIcon>
                    </Td>
                  </Tr>
                ))}
              </Tbody>
